`ifndef DISPATCHER_TOP_SV
`define DISPATCHER_TOP_SV
...
module dispatcher_top #(
    ...
    input  logic [9:0]                      wr_data,      // {y[4:0], x[4:0]}
    ...
    logic [4:0]                  matrix_check_x;
    logic [4:0]                  matrix_check_y;
    logic [8:0]                  matrix_neighborhood;
    logic                        matrix_collision;

    logic                        matrix_lock_en;
    logic [4:0]                  matrix_lock_x;
    logic [4:0]                  matrix_lock_y;

    logic                        matrix_release_en;
    logic [4:0]                  matrix_release_x;
    logic [4:0]                  matrix_release_y;
    ...
    logic [4:0] worker_coord_x [4];
    logic [4:0] worker_coord_y [4];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (int i = 0; i < 4; i++) begin
                worker_coord_x[i] <= 5'b0;
                worker_coord_y[i] <= 5'b0;
            end
        end else begin
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            matrix_release_x  <= 5'b0;
            matrix_release_y  <= 5'b0;
        end else begin
            // Default: no release
            matrix_release_en <= 1'b0;
    ...
    syndrome_fifo #(
        ...
        .wr_valid   (wr_valid),
        .wr_ready   (wr_ready),
        .wr_x       (wr_data[4:0]),      // Extract x from packed data
        .wr_y       (wr_data[9:5]),      // Extract y from packed data
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            ...
            if (issue_valid && |worker_issue) begin
                for (int i = 0; i < 4; i++) begin
                    if (worker_issue[i]) begin
                        $display("[Dispatcher] Cycle %0d: Issued syndr (%0d, %0d) to worker[%0d]",
                                 $time, issue_coord[4:0], issue_coord[9:5], i);
                    end
                end
            end
            ...
            for (int i = 0; i < 4; i++) begin
                if (worker_done[i]) begin
                    $display("[Dispatcher] Cycle %0d: Worker[%0d] DONE (active=%0b, stored_coord=(%0d,%0d))",
            ...
            if (matrix_release_en) begin
                for (int i = 0; i < 4; i++) begin
                    if (worker_done[i] && worker_active[i]) begin
                        $display("[Dispatcher] Cycle %0d: Release issued for worker[%0d] at (%0d,%0d)",

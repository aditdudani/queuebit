`ifndef DISPATCHER_FSM_SV
`define DISPATCHER_FSM_SV
...
module dispatcher_fsm #(
    ...
    input  logic [4:0]                      fifo_rd_x,
    input  logic [4:0]                      fifo_rd_y,

    // Tracking matrix interface
    output logic                            matrix_check_en,
    output logic [4:0]                      matrix_check_x,
    output logic [4:0]                      matrix_check_y,
    input  logic                            matrix_collision,

    output logic                            matrix_lock_en,
    output logic [4:0]                      matrix_lock_x,
    output logic [4:0]                      matrix_lock_y,

    output logic                            matrix_release_en,
    output logic [4:0]                      matrix_release_x,
    output logic [4:0]                      matrix_release_y,
    ...
    output logic [9:0]                      dispatch_coord, // 5-bit x + 5-bit y
    ...
    always_comb begin
        ...
        matrix_release_x   = 5'bx;
        matrix_release_y   = 5'bx;
        dispatch_valid     = 1'b0;
        worker_issue       = 4'b0000;
        dispatch_coord     = 10'bx;
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            ...
            if (current_state != next_state) begin
                $display("[FSM] Cycle %0d: %s → %s (fifo_valid=%0b, collision=%0b, workers_ready=%b)",
                         $time / 10,
                         current_state.name(),
                         next_state.name(),
                         fifo_rd_valid,
            ...
            if (matrix_check_en) begin
                $display("[FSM] Collision check: (%0d,%0d) → collision=%0b",
                         matrix_check_x, matrix_check_y, matrix_collision);
            end

            // Log lock operations
            if (matrix_lock_en) begin
                $display("[FSM] Lock issued: (%0d,%0d) → dispatch to worker", matrix_lock_x, matrix_lock_y);
            end

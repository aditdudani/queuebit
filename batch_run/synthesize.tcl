#!/usr/bin/env tclsh
# Synthesize d=11 dispatcher on XC7Z020 (ZedBoard)
# Includes constraint verification, synthesis & implementation, and power/timing analysis

package require Tcl 8.5

puts "=== d=11 Dispatcher Synthesis for XC7Z020 ==="
...
if {[string first "create_clock" $constraint_content] == -1} {
    puts "WARNING: No 'create_clock' found in XDC"
    puts "   Timing analysis may be unconstrained."
}

puts "Constraint file verified: $constraint_file"

# Create new Vivado project for d=11 synthesis
puts "\n[2/4] Creating Vivado project..."
set proj_dir "/tmp/queuebit_d11_synth"
set proj_name "queuebit_d11_synth"
...
add_files -norecurse [file normalize ../rtl/dispatcher_top.sv]
...
# Set top module
set_property top dispatcher_top [current_fileset]
...
open_run impl_1
report_timing_summary -file [file normalize build/timing_report.txt]
report_utilization -file [file normalize build/utilization_report.txt]
report_power -file [file normalize build/power_report.txt]

puts "Power report generated (post-implementation)"

# Optional: design hierarchy for verification
report_design -file [file normalize build/design_report.txt]

# Close project
close_project

puts "\n=== d=11 SYNTHESIS COMPLETE ==="

...
module tb_dispatcher_integration_naive #(
    ...
    // Dispatch log (NAIVE variant)
    string dispatch_log_filename = "dispatch_log_naive.txt";
    ...
    always_comb begin
        ...
        if (!wr_valid && stim_idx < stim_count) begin
        ...
        if (wr_valid && wr_ready) begin
            issued_this_cycle = 1;
        end else if (!wr_valid && wr_ready && stim_idx < stim_count && next_inject_credit_ppt >= 1000) begin
            issued_this_cycle = 1;
        end

        if (issued_this_cycle) begin
        ...
        inject_enable = (stim_idx < stim_count) && (next_inject_credit_ppt >= 1000) && wr_ready;
        inject_enable_next = ((stim_idx + 1) < stim_count) && (next_inject_credit_ppt >= 1000) && wr_ready;
    end
    ...
    // =========================================================================
    // Module Instantiation (NAIVE VARIANT - collision checking disabled)
    // =========================================================================

    dispatcher_top #(
        .ENABLE_COLLISION_CHECK(1'b0)  // DISABLED: measure collisions without avoidance
    ) dut (
        .clk            (clk),
        .rst_n          (rst_n),
        .wr_valid       (wr_valid),
    ...
    initial begin
        ...
        dispatch_log_fd = $fopen(dispatch_log_filename, "w");
        if (dispatch_log_fd == 0) begin
            $display("[WARNING] Could not open dispatch_log_naive.txt");
        end
        ...
        if (dispatch_log_fd != 0) begin
            ...
            $display("[Testbench] Dispatch log written to %s", dispatch_log_filename);
        end

        // Final report
        $display("\n");
        $display("================================================");
        $display("DISPATCHER INTEGRATION TEST RESULTS (NAIVE)");
        $display("================================================");
        ...
        $display("================================================");
        $display("Note: NAIVE variant has collision checking DISABLED");
        $display("Expected: verify ~25-40%% collisions detected via verify_collisions.py");
        $display("================================================\n");
